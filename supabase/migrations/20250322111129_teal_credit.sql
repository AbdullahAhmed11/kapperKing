/*
  # Fix Salon Settings Migration

  1. Changes
    - Add IF NOT EXISTS to policy creation
    - Keep existing table and policy structure
    - Add demo salon settings data
  
  2. Security
    - Maintain RLS on salon_settings table
    - Keep existing policies for data access
*/
...
-- Create salon settings table if it doesn't exist
...
DROP POLICY IF EXISTS "Salon settings are viewable by salon staff" ON public.salon_settings;
DROP POLICY IF EXISTS "Salon settings are manageable by salon admins" ON public.salon_settings;
