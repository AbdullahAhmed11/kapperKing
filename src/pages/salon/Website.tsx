...
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { Globe, Palette, Settings, Layout, Code, Share2, Plus, Trash2, Eye, Camera, Image as ImageIcon, Pencil } from 'lucide-react';
import Cookies from 'js-cookie'; 
import { jwtDecode } from 'jwt-decode';
...
const Website = () => {
  ...
  const [galleryImages, setGalleryImages] = useState<File[]>([]);
  const [uploading, setUploading] = useState(false);
  const [fetchedGalleryImages, setFetchedGalleryImages] = useState<GalleryImage[]>([]);
  ...
  useEffect(() => {
    if (activeTab === 'gallery' && decoded?.Id) {
      const fetchGalleryImages = async () => {
        try {
          const response = await fetch(`https://kapperking.runasp.net/api/Salons/GetMicrositeGallaries?micrositeId=${decoded.Id}`);
          if (response.ok) {
            const data = await response.json();
            setFetchedGalleryImages(data);
          } else {
            console.error('Failed to fetch gallery images:', await response.text());
          }
        } catch (error) {
  ...
  const handleImageUpload = async () => {
    ...
    try {
      ...
      if (response.ok) {
        alert('Gallery images uploaded successfully!');
        setGalleryImages([]);
        // Refresh fetched images after upload
        const fetchResponse = await fetch(`https://kapperking.runasp.net/api/Salons/GetMicrositeGallaries?micrositeId=${decoded.Id}`);
        if (fetchResponse.ok) {
  ...
  const handleEditImage = async (id: number, file: File) => {
    if (!file) return;
    ...
    try {
      const response = await fetch('https://kapperking.runasp.net/api/Salons/EditGallary', {
        method: 'POST',
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        // Refresh fetched images after edit
        const fetchResponse = await fetch(`https://kapperking.runasp.net/api/Salons/GetMicrositeGallaries?micrositeId=${decoded?.Id}`);
        if (fetchResponse.ok) {
          const data = await fetchResponse.json();
          setFetchedGalleryImages(data);
  ...
  const handleDeleteImage = async (id: number) => {
    try {
      const response = await fetch(`https://kapperking.runasp.net/api/Salons/DeleteGallary?id=${id}`, {
        method: 'DELETE',
      });

      if (response.ok) {
        alert('Image deleted successfully!');
        // Refresh fetched images after deletion
        const fetchResponse = await fetch(`https://kapperking.runasp.net/api/Salons/GetMicrositeGallaries?micrositeId=${decoded?.Id}`);
        if (fetchResponse.ok) {
          const data = await fetchResponse.json();
          setFetchedGalleryImages(data);
                ...
                <Input
                  ...
                  onChange={(e) => {
                    if (e.target.files && e.target.files[0]) {
                      const imageId = parseInt(e.target.dataset.imageId || '0');
                      handleEditImage(imageId, e.target.files[0]);
                    }
                  }}
                />
              </div>

              {(galleryImages.length > 0 || fetchedGalleryImages.length > 0) && (
                <>
                  {fetchedGalleryImages.length > 0 && (
                    <div className="mt-6">
                      <h4 className="text-md font-medium text-gray-900">Uploaded Images</h4>
                      <div className="mt-4 grid grid-cols-3 gap-4">
                        {fetchedGalleryImages.map((image) => (
                          <div key={image.id} className="relative group">
                            <img
                              src={`https://kapperking.runasp.net${image.image}`}
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  if (fileInputRef.current) {
                                    fileInputRef.current.dataset.imageId = image.id.toString();
                  ...
                  {galleryImages.length > 0 && (
                    <div className="mt-6">
                      <h4 className="text-md font-medium text-gray-900">Pending Upload Images</h4>
                      <div className="mt-4 grid grid-cols-3 gap-4">
                        {galleryImages.map((image, index) => (
                          <div key={index} className="relative group">
                            <img
