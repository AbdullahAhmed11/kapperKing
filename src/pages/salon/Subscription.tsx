...
import { toast } from 'sonner';
import { AlertCircle, CreditCard, CalendarDays, Loader2, Users, FileText, BarChartHorizontal, Edit, Trash2, Settings } from 'lucide-react';
...
export default function SalonSubscription() {
  ...
  const getSalonSubs = async () => {
    try {
      ...
      console.error(error);
      setError("Failed to fetch subscription data., faild");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const subscription = {
    ...
    usage: {
      staff: { current: 8, limit: 10 },
      appointments: { current: 450, limit: 500 },
      storage: { current: 4.2, limit: 5, unit: 'GB' },
    },
    paymentMethod: {
      brand: 'Visa',
  ...
  const handleCancelSubscription = async () => {
    ...
    if (!confirmed) return;
    try {
      await axios.post(`https://kapperking.runasp.net/api/Salons/CancelSubscription?subscriptionId=${subs.subscriptionId}`);
      toast.success("Subscription canceled successfully.");
      getSalonSubs();
    } catch (error) {
      console.error(error);
  ...
  // Function to generate and download PDF for an invoice
  const generateInvoicePDF = (invoice: any) => {
    const doc = new jsPDF();
    
    // Add logo (if available and accessible)
if (salonData?.image) {
      try {
        const logoUrl = `https://kapperking.runasp.net${salonData.image}`;
        doc.addImage(logoUrl, 'PNG', 160, 10, 30, 30); // Logo at top-right
      } catch (error) {
        console.error('Failed to add logo to PDF:', error);
        doc.setFontSize(10);
        doc.text('[Salon Logo]', 160, 20); // Fallback placeholder
      }
    } else {
      doc.setFontSize(10);
      doc.text('[Salon Logo]', 160, 20); // Fallback if no image
    }

    // Add salon details
    doc.setFontSize(14);
    doc.text(salonData?.name || 'Salon Manager Pro', 20, 20);
    doc.setFontSize(10);
    doc.text(`Email: ${salonData?.email || 'billing@salonmanager.com'}`, 20, 28);
    doc.text(`Phone: ${salonData?.salonPhone || 'N/A'}`, 20, 34);
    doc.text(`Address: ${salonData?.address || 'N/A'}`, 20, 40);
    doc.text(`City: ${salonData?.city || 'N/A'}`, 20, 46);
    doc.text(`Postal Code: ${salonData?.postalCode || 'N/A'}`, 20, 52);
    doc.text(`Country: ${salonData?.country || 'N/A'}`, 20, 58);
    doc.text(`Invoice Date: ${new Date(invoice.createAt).toLocaleDateString()}`, 20, 64);
    
    // Add invoice title
    doc.setFontSize(18);
    doc.text(`Invoice ${invoice.id}`, 20, 76);
    ...
    autoTable(doc, {
      startY: 86,
      head: [['Invoice ID', 'Subscription', 'Amount', 'Date', 'Status']],
      body: tableData,
      theme: 'striped',
      headStyles: { fillColor: [79, 70, 229] }, // Indigo color for header
      styles: { fontSize: 10, cellPadding: 3 },
      columnStyles: {
        0: { cellWidth: 40 }, // Invoice ID
        1: { cellWidth: 50 }, // Subscription
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex justify-between items-start p-4 bg-indigo-50 rounded-lg border border-indigo-100">
                <div>
                  <h4 className="font-semibold text-lg text-indigo-800">{subs?.name}</h4>
                  <p className="text-sm text-indigo-600">{subs?.price || 'N/A'}</p>
                </div>
                <div className="flex space-x-2">
                  <Button variant="outline" size="sm" onClick={handleChangePlan}>Change Plan</Button>
              ...
              <div className="space-y-3 pt-4">
                <UsageBar label="Staff Members" current={subs?.stylistCount ?? 0} limit={subs?.staffLimit ?? 0} />
                <UsageBar label="Monthly Appointments" current={subscription.usage.appointments.current} limit={subs?.customerLimit ?? 0} />
                <UsageBar label="Customers" current={subs?.customers ?? 0} limit={subs?.customerLimit ?? 0} />
              </div>
            </CardContent>
          </Card>
