...
import { zodResolver } from '@hookform/resolvers/zod'; // Restore
import { z } from 'zod';
...
export function PageForm({ open, onClose, onSubmit, initialData, title = 'Add Page' }: PageFormProps) {
  // Determine default values based on initialData *before* calling useForm
  // Determine default values based on initialData *before* calling useForm
  // Ensure types match PageFormData exactly
  const formDefaultValues: PageFormData = initialData
    ? {
        title: initialData.title,
        type: initialData.type, // Type should already be correct from FullPageData
        content: initialData.content ?? '',
        status: initialData.status, // Type should already be correct
        meta: {
    ...
    : { // Defaults for new page
        title: '',
        type: 'page', // Explicitly 'page' which matches the enum
        content: '',
        status: 'draft', // Explicitly 'draft' which matches the enum
        meta: { description: '', keywords: '' },
      };

  const { register, handleSubmit, formState: { errors, isSubmitting }, reset, control, setValue } = useForm<PageFormData>({ // Add setValue
    resolver: zodResolver(pageSchema), // Restore resolver
    defaultValues: formDefaultValues // Keep initializing with defaults
  });

  // Use useEffect to explicitly set values when initialData changes
  useEffect(() => {
    if (initialData) {
      console.log("PageForm: Setting values with initialData:", initialData);
      setValue('title', initialData.title);
      setValue('type', initialData.type);
      setValue('content', initialData.content ?? '');
      setValue('status', initialData.status);
      setValue('meta.description', initialData.meta?.description || '');
      setValue('meta.keywords', initialData.meta?.keywords || '');
    } else {
      // Optionally reset fields if needed when initialData becomes null (e.g., closing edit)
      // reset(formDefaultValues); // Or reset specific fields
      console.log("PageForm: initialData is null, potentially resetting.");
      // You might want to reset to empty defaults here if the form instance is reused
      // For now, relying on the key prop to handle this.
    }
    // Depend only on initialData; setValue is stable.
