...
const EditSalonForm = ({ open, onClose, salonId, onSuccess }: EditSalonFormProps) => {
  ...
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [salonPlanName, setSalonPlanName] = useState<string | null>(null);
    ...
    const fetchSalonData = async () => {
      ...
      try {
        ...
        reset({
          ...
          ownerId: salonData.ownerId?.toString() || '',
          subscriptionId: salonData.subscriptionId?.toString() || '',
          longitude: salonData.longitude?.toString() || '',
          latitude: salonData.latitude?.toString() || '',
          website: salonData.website || '',
        });

        setSalonPlanName(salonData.planName || null);
      } catch (error) {
        console.error(error);
        toast.error('Failed to load salon data');
      } finally {
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              ...
              >
                ...
                {/* fallback option if current plan is missing */}
                {!plans.find((p) => p.id.toString() === watch('subscriptionId')) &&
                  watch('subscriptionId') &&
                  salonPlanName && (
                    <option value={watch('subscriptionId')}>{salonPlanName}</option>
                  )}
